...
const AllNews: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.API_URL}/game-news?game=${game_id ?? ""}&search=${
          searchs ?? ""
        }`
      )
      .then((res) => {
    ...
    <>
      ...
      <div className="relative">
        ...
        <div className="absolute w-full h-[300px] px-12 md:px-0 flex items-center justify-center z-[12] top-1">
          <div className="flex shadow border-gray-400 bg-white items-center rounded-[10px] font-[Candara] overflow-hidden w-full md:w-[650px] lg:min-w-[975px]">
            <select
              value={game_id}
              className={
                "p-2 font-[600] text-center focus:outline-none w-[100px] md:w-[200px]"
              }
              onChange={(e) => {
                router.replace({
                  query: {
            ...
            >
              ...
              {landing?.game.map((row) => (
                <option className="" key={row.id} value={row.id}>
                  {row.name}
                </option>
              ))}
            </select>
            <input
              ...
              placeholder={"Search"}
              className={
                "font-sans border-l border-gray-500 font-[600] p-2 w-full focus:outline-none"
              }
              type="search"
            />
            <button
      ...
      <div className="py-8 md:py-[150px] lg:container px-4 md:px-4 xl:max-w-[1100px] mx-auto">
        {listNews &&
          listNews.map((row: any, key: number) => (
            <Link key={row.id} href={`/games/detail/${row.id}`}>
              <div
                key={row.id}
                className={`md:flex items-center rounded-[20px] overflow-hidden shadow-lg font-[Candara] ${
                  key > 0 ? " mt-[25px] md:mt-[50px]" : ""
                }`}
              >
                <div className="">
                  <Image layout="fixed"
                ...
                <div className="bg-white w-full md:h-[260px] p-4">
                  <p className={"font-[700] text-[#5F6368] text-[24px]"}>
                    {row.title}
                  </p>
                  <div className="flex gap-4 mt-2">
                    <p className="px-[12px] py-[10px] bg-red-500 text-white font-[400]">
                      {row.category.name}
                    </p>
                    <p className=""></p>
                  </div>
