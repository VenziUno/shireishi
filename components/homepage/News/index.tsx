...
const CustomPaginate = (i: number, active: any) => {
  return (
    <div className={"flex items-center justify-center h-[50px] w-[50px] mt-10"}>
      {i === active ? (
...
const NewsComponent: React.FC<props> = ({ news, blogCategory }) => {
  ...
  return (
    <>
      <div
        className="mx-auto md:w-[400px] w-full px-14 md:p-3 mb-12"
        id={"news"}
      >
        <h2
          className={
            "uppercase md:text-2xl mx-auto sm:text-xl text-lg text-white flex items-center justify-center games__background py-6 drop-shadow-[0_7px_4px_rgba(0,0,0,0.4)] lg:w-fit w-full lg:px-36"
          }
        >
          News
        </h2>
      ...
      <div className="custom__container py-10">
        <div className="flex gap-4 md:gap-0 md:justify-between items-center px-4">
          <div className="flex flex-grow gap-3 items-center font-sans overflow-x-auto whitespace-nowrap scroll__custom">
            <button
              onClick={() => filterByCategoryId(0)}
              className={`border-1 ${
                activeCategory === 0 ? "bg-black text-white" : ""
              } border border-gray-300 rounded px-5 py-2 bg-white`}
            >
              All
            </button>
            {blogCategory.map((row) => (
              <button
                ...
                className={`border-1 border border-gray-300 rounded px-5 py-2 bg-white ${
                  activeCategory === row.id ? "bg-black text-white" : ""
                }`}
              >
                #{row.name}
              </button>
          ...
          <div className="whitespace-nowrap hidden lg:block">
            <Link href={"/all-blog"}>
              <button
                className={
                  "bg-black text-center font-[Candara] font-[600] p-2 text-white md:px-10 rounded-full"
                }
              >
                See All Blog
              </button>
        ...
        <div className="block lg:hidden mt-8 px-4">
          <Link href={"/all-blog"} className={"flex items-center"}>
            <button
              className={
                "mx-auto bg-black text-center font-[Candara] font-[600] p-2 px-10 text-white md:px-10 rounded-full"
              }
            >
              See All Blog
            </button>
        ...
        <div className="my-12 news-slick">
          <Slider
            slidesToShow={Math.max(listNews.length > 2 ? 2 : listNews.length, 1)}
            dots={true}
          ...
          >
            {listNews &&
              listNews.map((row) => (
                <div className={"px-5 w-full"} key={row.id} >
                  <div
                    key={row?.id}
                  ...
                  >
                    <div className="w-full h-full mx-auto rounded-[10px]">
                      <Image
                        ...
                        height={250}
                        className={
                          "w-full h-[250px] md:h-[300px] object-cover shadow mx-auto "
                        }
                      />
                    </div>
                    <div className="">
                      <p
                        className={
                          " text-[18px] text-1-line md:text-[28px] font-bold font-sans mt-6"
                        }
                      >
                        {row.title}
                      </p>
                      ...
                      <p
                        className=" my-6 whitespace-pre-wrap break-words text-4-line overflow-auto"
                        dangerouslySetInnerHTML={{ __html: row.body }}
                      />
                      <Link
                        href={`all-blog/${row.id}?category=${row.category.id}`}
